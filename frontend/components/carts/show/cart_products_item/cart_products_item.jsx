    ...
    for (let i = 1; i < 14; i++) {
      if (i === this.props.product.quantity) {
        dropDownOptions.push(
          <option value={`${i}`} key={`quantity-option-${i}`} selected>
            {i}
          </option>
        );
      } else {
        dropDownOptions.push(
          <option value={`${i}`} key={`quantity-option-${i}`}>
            {i}
          </option>
        );
      }
      ...
      <div className="cart-products-item">
        ...
        <div className="cart-products-item-details">
          <div className="cart-products-item-title-and-remove">
            <div>
              <Link
                className="cart-products-item-title-link"
                to={`/products/${product.id}`}
              >
                <p className="cart-products-item-title">{product.title}</p>
              </Link>
            </div>
